...
def main():
    ...
    if not repo_id:
        raise ValueError("repo_id no fue inicializado correctamente. No se pudo crear el repositorio.")
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        # **Aquí va el código para subir el modelo al final de la última época** (código nuevo que debes agregar)
        if epoch == args.num_train_epochs - 1:  # Solo al final del entrenamiento
            accelerator.wait_for_everyone()
            unwrapped_model = accelerator.unwrap_model(model)
            model_id = f"model-{wandb.run.id}"  # Usa el id único de wandb para nombrar el modelo

            # Crea un nuevo repositorio en Hugging Face para este run específico
            if accelerator.is_main_process:
                repo_name = f"landmines-detection-{wandb.run.id}"  # Nombre único para el repo
                repo_id = api.create_repo(repo_name, exist_ok=True, token=args.hub_token).repo_id

                # Guardar el modelo
                unwrapped_model.save_pretrained(
                    os.path.join(args.output_dir, model_id),
                    is_main_process=accelerator.is_main_process,
                    save_function=accelerator.save
                )

                # Guardar el processor también si es necesario
                image_processor.save_pretrained(os.path.join(args.output_dir, model_id))

                # Subir a Hugging Face al nuevo repo creado
                api.upload_folder(
                    commit_message=f"Model trained with learning_rate {wandb.config.learning_rate}",
                    folder_path=os.path.join(args.output_dir, model_id),
                    repo_id=repo_id,  # Usa el repo_id recién creado para este run
                    repo_type="model",
                    token=args.hub_token,
                )

        # Guardar el modelo al final de cada época si se usa checkpointing por época
